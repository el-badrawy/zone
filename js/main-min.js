...
async function loadLang(t) {
  ...
  (document.querySelectorAll("[data-key]").forEach((t) => {
    ...
    (document.body.dir = "ar" === t ? "rtl" : "ltr"));
}
(document.addEventListener("DOMContentLoaded", loadLang(currentLang)),
  ...
  document.getElementById("langSwitch").addEventListener("change", (t) => {
    loadLang(t.target.value);
  }));
...
function setActiveCategory() {
  ...
  ("" === t && (t = "index.html"),
    document.querySelectorAll(".cat").forEach((t) => {
      t.classList.remove("active");
    }),
    document.querySelectorAll(".cat a").forEach((e) => {
      e.getAttribute("href") === t && e.parentElement.classList.add("active");
    }));
}
...
function update() {
  cards.forEach((t, e) => {
    (t.classList.remove("left", "center", "right", "hidden", "left2", "right2"),
      e === current
        ? t.classList.add("center")
        : e === current - 1
          ? t.classList.add("left")
          : e === current - 2 && window.innerWidth > 767
            ? t.classList.add("left2")
            : e === current + 1
              ? t.classList.add("right")
              : e === current + 2 && window.innerWidth > 767
                ? t.classList.add("right2")
...
(update(),
  ...
  (document.getElementById("next").onclick = () => {
    (current++,
      current >= cards.length && (current = 0),
      stopAuto(),
      update(),
      restartAuto());
  }),
  (document.getElementById("prev").onclick = () => {
    (current--,
      current < 0 && (current = cards.length - 1),
      stopAuto(),
      update(),
      restartAuto());
  }));
let startX = 0,
  endX = 0;
const container = document.querySelector(".moreSell");
(container.addEventListener("touchstart", (t) => {
  startX = t.touches[0].clientX;
}),
  container.addEventListener("touchmove", (t) => {
  ...
  container.addEventListener("touchend", () => {
    ...
    Math.abs(t) > 50 &&
      (t > 0
        ? (current++, current >= cards.length && (current = 0))
        : (current--, current < 0 && (current = cards.length - 1)),
      update(),
      stopAuto(),
      restartAuto());
  }));
